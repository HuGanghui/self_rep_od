...
if __name__ == '__main__':
    # 多进程利用多个GPU训练模型
    process_list = []
    data_list = ["0th_ts_train", "apascal", "bank-additional-full_normalised", "lung-1vs5", "probe", "secom"]
    ...
    for i in range(4):
        p = Process(target=fun1, args=(docker_command_list[i].format(GPU=i, data=data_list[0]),))
        p.start()
        process_list.append(p)
